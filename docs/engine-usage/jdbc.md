...
This article mainly introduces the installation, use and configuration of the JDBC engine plug-in in Linkis.

## 1. Preliminary work
### 1.1 Environment Installation

If you want to use JDBC engine on your server, you need to prepare JDBC connection information, such as MySQL database connection address, user name and password, etc.
...
For JDBC tasks, you only need to modify the EngineConnType and CodeType parameters in the Demo:
...
[Scriptis](https://github.com/WeBankFinTech/Scriptis) is the easiest to use. You can directly enter Scriptis, right-click the directory, create a new JDBC script, write the JDBC code, and click to execute.

The execution principle of JDBC is to load the JDBC driver and then submit the sql to the SQL server to execute and obtain the result set and return it.

![](./images//jdbc-run.png)

### 3.4 Multiple data source support
...
### 4.1 Default configuration instructions
...
| wds.linkis.jdbc.driver | none | yes | jdbc link driver |
| wds.linkis.jdbc.username | none | yes | database connection username |
| wds.linknis.jdbc.password | none | yes | database link password |
| wds.linkis.jdbc.connect.max | 10 | no | jdbc engine maximum number of connections |
...
insert into `linkis_ps_configuration_config_key` (`key`, `description`, `name`, `default_value`, `validate_type`, `validate_range`, `is_hidden`, `is_advanced`, `level`, `treeName`, `engine_conn_type`) VALUES ('wds.linkis.jdbc.version', 'value range: jdbc3,jdbc4', 'jdbc version','jdbc4', 'OFT', '[\"jdbc3\",\"jdbc4\"]' , '0', '0', '1', 'User Configuration', 'jdbc');
