...
> Linkis provides convenient JAVA and SCALAA calls, which can be used only by introducing linkis-computation-client modules, adding new support with Label submissions after 1.0 and described below for 0.X and 1.0.

## 1 Import Dependencies Module
```
<dependency>
  <groupId>com.webank.wedatasphere. inkis</groupId>
  <artifactId>linkis-compution-client</artifactId>
  <version>${linkis.version}</version>
</dependency>
like：
<dependency>
  <groupId>com. ebank.wedatasphere.linkis</groupId>
  <artifactId>linkis-compution-client</artifactId>
  <version>1.0.0.0-RC1</version>
</dependency>
```

## 2 Execute submission compatible with 0.X
### 2.1 Java test code
Create a test class in Java, UJESClientImplTestJ, the meaning of a specific interface can be found in comment：
```java
...
## 3.0 New Submission Method
1.0 New client.submitting method for interfacing with 1.0 new task implementation interfaces to support incoming Label, etc.
### 3.1 Java test class
```
package com.webank.wedatasphere.linkis.client.test;
