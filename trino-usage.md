...
```
并重启linkis-manager或者通过引擎接口进行刷新(linkis-engineplugin服务已合并到linkis-manager服务中)
```bash
cd ${LINKIS_HOME}/sbin
...
```shell
sh ./bin/linkis-cli -submitUser hadoop -engineType trino-371 -codeType trino -code 'select * from system.jdbc.schemas limit 10' -runtimeMap linkis.trino.url=http://127.0.0.1:8080
```
