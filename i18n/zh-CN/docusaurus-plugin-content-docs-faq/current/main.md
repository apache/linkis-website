...
### Q8: 流式集群跑Spark任务
在流式集群中跑Spark任务会报java.lang.NoSuchFieldException: DEFAULT_TINY_CACHE_SIZE错误，错误信息如下。
```java
...
### Q35: 运行Spark、Flink等需要使用YARN作为资源调度的任务时报Deployment took more than 60 seconds，Please check if the requested resources. 

![](/faq/q32_3.png)

原因：1.yarn链接参数配置有问题 2.队列资源不足

解决办法：

1.yarn链接参数配置问题

检查linkis_cg_rm_external_resource_provider表配置的yarn链接参数是否有效。正确的配置如下：
![](/faq/q35_32.png)

2.队列资源不足问题

参考管理台使用中的[队列设置](https://linkis.apache.org/zh-CN/docs/latest/user-guide/control-panel/param-conf#51-%E9%98%9F%E5%88%97%E8%AE%BE%E7%BD%AE)，设置可用的队列。